...
0 < 
	State.Text -> State.BeforeTagName 
	sectionStart = this.index = 0
1 d
	sectionStart = this.index = 1
	State.BeforeTagName -> State.InTagName
23 iv
	
4 > 
	cbs.onopentagname => currentOpenTag
		name = getSlice(this.sectionStart, this.index)
		==> getSlice(1, 4) => 'div'
	sectionStart = -1
	State.inTagName -> State.BeforeAttrName
	c === '>' ?
		=> cbs.onopentagend 
			=> addNode(currentOpenTag) 
		-> stack.unshift(currentOpenTag)
	currentOpenTag = null
	State.BeforeAttrName -> State.Text 
	sectionStart = this.index + 1 = 5
567 hi,  

8 {  
	State.Text -> State.InterpolationOpen 
	delimiterIndex = 0
	delimiterIndex++
9 {
	start = this.index + 1 - this.delimiterOpen.length
		=> 9 + 1 - 2 => 8
	cbs.ontext
		-> getSlice(this.sectionStart, start)
		=> getSlice(5, 8) => hi,
		.children.push(textNode)
	State.InterpolationOpen -> State.Interpolation
	sectionStart = start = 8
10 11 12 13 14 15 16 message
	
17 }
	State.Interpolation -> State.InterplocationClose
	delimiterIndex = 0
	delimiterIndex++
18 }
	delimiterIndex = 1
	cbs.oninterpolation
		=> oninterpolation(this.sectionStart, this.index + 1)
			innerStart = this.sectionStart + '{{'.length
			innerEnd = this.index + 1 - '}}'.length
			exp = getSlice(innerStart, innerEnd)
				=> getSlice(10, 17)
				=> message
			addNode()
			State.InterplocationClose -> State.Text
			sectionStart = this.index + 1 => 19
19 <
	State.Text -> State.BeforeTagName
	sectionStart = this.index = 19
20 /
	State.BeforeTagName -> State.BeforeClosingTagName
21 d
	State.BeforeClosingTagName -> State.InClosingTagName
	sectionStart = this.index = 21
22 23 iv

24 >
	cbs.onclosetag(this.sectionStart, this.index)
		=> getSlice(this.sectionStart, this.index)
			=> getSlice(21, 24) => div
	sectionStart = -1
	State.InClosingTagName -> State.AfterClosingTagName
	c === > ?
		State.AfterClosingTagName -> State.Text
